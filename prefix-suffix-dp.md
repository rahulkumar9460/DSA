...
**Pattern Recognition**
```
This is a classic transformation trick:

Split expression into:
(left part depending on i) + (right part depending on j)

Expression like: A[i] + A[j] + f(i, j)
Rewrite into: (A[i] + something with i) + (A[j] + something with j)
```
...
int maxScoreSightseeingPair(vector<int>& values) {
    ...
    // keep iterating j, make best, update ans
